...
type paymentsService interface {
	InitializedOrderPayment(id orders.ID, price price.Price) error
}
...
func NewOrdersService(productsService productsService, paymentsService paymentsService, orderRepository orders.Repository) OrdersService {
	return OrdersService{productsService, paymentsService, orderRepository}
}
...
func (s OrdersService) PlaceOrder(cmd PlaceOrderCommand) error {
	...
	if err != nil {
		return errors.Wrap(err, "cant get product")
	}

	newOrder, err := orders.NewOrder(cmd.OrderID, product, address)
	if err != nil {
		return errors.Wrap(err, "cant create order")
	}

	if err := s.ordersRepository.Save(newOrder); err != nil {
		return errors.Wrap(err, "cant save order")
	}

	if err := s.paymentsService.InitializedOrderPayment(newOrder.ID(), newOrder.Product().Price()); err != nil {
		return errors.Wrap(err, "Cant initialise payment")
	}
...
func (s OrdersService) MarkOrderAsPaid(cmd MarkOrderAsPaidCommand) error {
	...
	if err != nil {
		return errors.Wrapf(err, "cant get order %s", cmd.OrderID)
	}
...
func (s OrdersService) OrderByID(id orders.ID) (orders.Order, error) {
	...
	if err != nil {
		return orders.Order{}, errors.Wrapf(err, "cant get order %s", id)
	}
	return *o, nil
}
